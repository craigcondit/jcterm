...
loop:
	  ...
	  if(buf[0]==0x0d){ out.write(lf, 0, lf.length); }
	  else if(buf[0]<0x20 || (buf[0]&0x80)!=0){ continue; }
	  else{ out.write(buf, 0, i); }
	  out.flush();

	  for(int j=0;j<i;j++){
            sb.append((char)buf[j]);
            if(buf[j]==0x0d){
	      System.arraycopy(sb.toString().getBytes(), 0, 
			       buf, 0, sb.length());
	      i=sb.length();
	      break loop;
	    } 
	...
	if(cmd.equals("cd") || cmd.equals("lcd")){
	  ...
	  try{
	    ...
	    out.write(e.message.getBytes());
	    out.write(lf); out.flush();
	  }
	  continue;
	}
	...
	if(cmd.equals("pwd") || cmd.equals("lpwd")){
	  ...
	  //out.print(str+"\n");
          out.write(str.getBytes()); out.write(lf); out.flush();
	  continue;
	}
...
//	  out.print(help+"\n");
	    ...
	    out.write((help[j]).getBytes()); out.write(lf); 
	  }
	  out.flush();
	  continue;
	}
...
//      monitor.close();
      ...
      status();
      try{out.write((byte)0x0d); out.write((byte)0x0a); out.flush();}
      catch(Exception e){}
    }
    private void status(){
      try{
